...
export function PwaCameraCaptureModal({
  ...
  // Keep onError stable so the camera useEffect doesn't restart on every parent re-render
  ...
  useEffect(() => {
    ...
    // onError intentionally excluded — stable via onErrorRef to prevent camera restarts
  ...
  // Handle gallery photo selection — stops camera stream then passes file up as if it were captured
  const handleGallerySelect = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;

    console.log('[PWA Camera] Gallery photo selected:', file.name);
          ...
          <>
            ...
            <div className="absolute bottom-0 left-0 right-0 z-10 pb-8 safe-area-inset-bottom">
              <div className="flex items-center justify-center gap-8 px-8">
                {/* Gallery Button — always shown on the left */}
                <label
                ...
                {/* Flip Camera Button (right side) — shown only if multiple cameras */}
                {hasMultipleCameras ? (
                  <motion.button
                  ...
                  <div className="w-14 h-14" /> // Spacer to keep shutter centred
                )}
              </div>
            </div>
