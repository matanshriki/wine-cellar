    ...
    const openaiResponse = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `You are a wine label OCR and data extraction expert. 
  ...
  "alcohol": { "value": 13.5 or null, "confidence": "high"|"medium"|"low" }
          ...
          },
          {
            ...
            content: [
              {
                type: 'text',
                text: 'Extract wine data from this bottle label image. Return only the JSON response.'
              },
              {
                type: 'image_url',
                image_url: {
    ...
    const parsedData: ParsedWineData = JSON.parse(cleanContent);
